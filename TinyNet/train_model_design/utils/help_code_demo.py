...
def loadCSV(csvf):
    ...
    with open(csvf) as csvfile:
        ...
        next(csvreader, None)  # skip (label, filename)
        for i, row in enumerate(csvreader):
            ...
            if label in dictLabels.keys():
                dictLabels[filename].append(label)
            else:
                dictLabels[filename] = [label]
...
class IEGM_DataSET():
    ...
    def __getitem__(self, idx):
        ...
        val = self.names_list[idx].split(' ')[0]
        seg = val.rsplit('.', 1)[0].split('-')
        ...
        sample = {'IEGM_seg': IEGM_seg, 'label': label, 'parsed_filename': seg}
